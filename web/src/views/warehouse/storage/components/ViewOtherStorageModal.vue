<template>
  <BasicModal v-bind="$attrs" @register="registerModal" :title="getTitle" @ok="handleSubmit">
    <div class="components-page-header-demo-responsive" style="border: 1px solid rgb(235, 237, 240)">
      <a-page-header
          :title="t('warehouse.otherStorage.detailOtherStorage')"
          :sub-title= "receiptNumber">
        <template #extra>
          <a-button @click="exportTable">{{ t('warehouse.otherStorage.export.name') }}</a-button>
        ...
        <a-descriptions size="small" :column="3">
          <a-descriptions-item :label="t('warehouse.otherStorage.header.supplierName')">{{ supplierName }}</a-descriptions-item>
          <a-descriptions-item :label="t('warehouse.otherStorage.header.receiptDate')">{{ receiptDate }}</a-descriptions-item>
          <a-descriptions-item ></a-descriptions-item>
          <a-descriptions-item :label="t('warehouse.otherStorage.header.remark')">
            {{ remark }}
          </a-descriptions-item>
        </a-descriptions>
        <div class="extra">
          ...
          >
            <a-statistic
                :title="t('warehouse.otherStorage.header.status')"
...
export default defineComponent({
  ...
  setup() {
    ...
    const [registerTable] = useTable({
      title: t('warehouse.otherStorage.export.exportData'),
    ...
    async function exportTable() {
      ...
      if (file.size > 0) {
        ...
        link.download = t('warehouse.otherStorage.export.exportData') + timestamp + ".xlsx";
        link.target = "_blank";
        link.click();
      }
    }
  ...
  <div style="${flexContainer}">
    <div style="${flexItem}">${t('warehouse.otherStorage.header.receiptNumber')}：${receiptNumber.value}</div>
    <div style="${flexItem}">${t('warehouse.otherStorage.header.receiptDate')}：${receiptDate.value}</div>
    <div style="${flexItem}">${t('warehouse.otherStorage.header.supplierName')}：${supplierName.value}</div>
    <div style="${flexItem}">${t('warehouse.otherStorage.header.remark')}：${remark.value}</div>
  </div>

`;
      printJS({
        documentTitle: "EAIRP " + t('warehouse.otherStorage.detailReceipt'),
        header: header,
