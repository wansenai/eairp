<template>
  <div>
    <BasicTable @register="registerTable">
      <template #toolbar>
        <a-button type="primary" @click="handleCreate" v-text="t('warehouse.allotShipments.add')" />
        <a-button type="primary" @click="handleBatchDelete" v-text="t('warehouse.allotShipments.batchDelete')" />
        <a-button type="primary" @click="handleExport" v-text="t('warehouse.allotShipments.export.name')" />
...
export default defineComponent({
  name: 'AllotShipments',
  components: {'a-modal': Modal, 'a-checkbox': Checkbox, Tag, TableAction, BasicTable, AddEditAllotShipmentsModal, ViewAllotShipmentsModal},
  setup() {
    const { t } = useI18n();
    ...
    const [registerTable, { reload, getSelectRows, getForm, getDataSource }] = useTable({
      title: t('warehouse.allotShipments.title'),
      rowKey: 'id',
    ...
    async function handleExport() {
      ...
      if (dataSum.value === 0) {
        createMessage.warn(t('warehouse.allotShipments.export.noData'));
        return;
      }
