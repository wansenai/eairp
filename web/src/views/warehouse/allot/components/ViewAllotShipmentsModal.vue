<template>
  <BasicModal v-bind="$attrs" @register="registerModal" :title="getTitle" @ok="handleSubmit">
    <div class="components-page-header-demo-responsive" style="border: 1px solid rgb(235, 237, 240)">
      <a-page-header
          :title="t('warehouse.allotShipments.detailAllotShipments')"
          :sub-title= "receiptNumber">
        <template #extra>
          <a-button @click="exportTable">{{ t('warehouse.allotShipments.export.name') }}</a-button>
        ...
        <a-descriptions size="small" :column="3">
          <a-descriptions-item :label="t('warehouse.allotShipments.header.receiptDate')">{{ receiptDate }}</a-descriptions-item>
          <a-descriptions-item :label="t('warehouse.allotShipments.header.remark')">
            {{ remark }}
          </a-descriptions-item>
        </a-descriptions>
        <div class="extra">
          ...
          >
            <a-statistic
                :title="t('warehouse.allotShipments.header.status')"
...
import {exportAllotShipmentsDetail, getAllotShipmentsDetailById} from "@/api/warehouse/allotShipments";
...
export default defineComponent({
  ...
  setup() {
    ...
    const [registerTable] = useTable({
      title: t('warehouse.allotShipments.export.exportData'),
    ...
    async function exportTable() {
      const file: any = await exportAllotShipmentsDetail(receiptNumber.value)
      if (file.size > 0) {
        const blob = new Blob([file]);
        const link = document.createElement("a");
        const timestamp = getTimestamp(new Date());
        link.href = URL.createObjectURL(blob);
        link.download = t('warehouse.allotShipments.export.exportData') + timestamp + ".xlsx";
        link.target = "_blank";
        link.click();
      }
    }
  ...
  <div style="${flexContainer}">
    <div style="${flexItem}">${t('warehouse.allotShipments.header.receiptNumber')}：${receiptNumber.value}</div>
    <div style="${flexItem}">${t('warehouse.allotShipments.header.receiptDate')}：${receiptDate.value}</div>
    <div style="${flexItem}">${t('warehouse.allotShipments.header.remark')}：${remark.value}</div>
  </div>

`;
      printJS({
        documentTitle: "EAIRP " + t('warehouse.allotShipments.detailReceipt'),
        header: header,
