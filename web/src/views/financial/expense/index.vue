<template>
  <div>
    <BasicTable @register="registerTable">
      <template #toolbar>
        <a-button type="primary" @click="handleCreate" v-text="t('financial.expense.add')" />
        <a-button type="primary" @click="handleBatchDelete" v-text="t('financial.expense.batchDelete')" />
        <a-button type="primary" @click="handleExport" v-text="t('financial.expense.export.name')" />
        ...
        <template v-if="column.key === 'action'">
          <TableAction
              :actions="[
              {
                ...
                tooltip: t('financial.expense.viewExpenseReceiptDetail'),
...
export default defineComponent({
  ...
  setup() {
    ...
    async function handleBatchDelete() {
      ...
      const result = await deleteBatchExpense(ids);
    ...
    async function handleDelete(record: Recordable) {
      const result = await deleteBatchExpense([record.id]);
      if (result.code === 'E0006') {
        await reload();
      }
    }
    ...
    async function handleOnStatus(newStatus: number) {
      ...
      const ids = data.map((item) => item.id);
      const {code} = await updateExpenseStatus(ids, newStatus);
      if (code === "E0005") {
        await reload();
      }
    }
    ...
    async function handleExport() {
      ...
      if (dataSum.value === 0) {
        createMessage.warn(t('financial.expense.export.noData'));
        return;
      }
    ...
    const handleExportOk = async () => {
      ...
      if (file.size > 0) {
        ...
        link.download = t('financial.expense.expenseReceiptDetail') + timestamp + ".xlsx";
        link.target = "_blank";
        link.click();
      }
      confirmLoading.value = false;
