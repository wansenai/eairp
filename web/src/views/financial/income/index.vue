<template>
  <div>
    <BasicTable @register="registerTable">
      <template #toolbar>
        <a-button type="primary" @click="handleCreate" v-text="t('financial.income.add')" />
        <a-button type="primary" @click="handleBatchDelete" v-text="t('financial.income.batchDelete')" />
        <a-button type="primary" @click="handleExport" v-text="t('financial.income.export.name')" />
        ...
        <template v-if="column.key === 'action'">
          <TableAction
              :actions="[
              {
                ...
                tooltip:  t('financial.income.viewIncomeReceiptDetail'),
...
export default defineComponent({
  ...
  setup() {
    ...
    const [registerTable, { reload, getSelectRows, getForm, getDataSource }] = useTable({
      title: t('financial.income.title'),
      rowKey: 'id',
    ...
    async function handleBatchDelete() {
      ...
      const result = await deleteBatchIncome(ids);
      if (result.code === 'I0006') {
        await reload();
      }
    }
    ...
    async function handleDelete(record: Recordable) {
      const result = await deleteBatchIncome([record.id]);
      if (result.code === 'I0006') {
        await reload();
      }
    }
    ...
    async function handleOnStatus(newStatus: number) {
      ...
      const ids = data.map((item) => item.id);
      const {code} = await updateIncomeStatus(ids, newStatus);
      if (code === "I0005") {
        await reload();
      }
    }
    ...
    async function handleExport() {
      ...
      if (dataSum.value === 0) {
        createMessage.warn(t('financial.income.export.noData'));
        return;
      }
    ...
    const handleExportOk = async () => {
      ...
      if (file.size > 0) {
        ...
        link.download = t('financial.income.incomeReceiptDetail') + timestamp + ".xlsx";
        link.target = "_blank";
        link.click();
      }
      confirmLoading.value = false;
